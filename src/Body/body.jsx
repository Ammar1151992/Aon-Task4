                ...
                <div className="table">
                    <table>
                        ...
                        {products.map((item, index) => (
                            <tr>
                            <td><span className="onMobile">ID: </span>{item.id}</td>
                            <td><span className="onMobile">Title: </span> {item.title}</td>
                            <td><span className="onMobile">Description: </span>
                             {item.description.slice(0, txtLength)}
                             {item.description.length > txtLength && (
                                <button className="txt" onClick={() => {
                                    setShowTxt(true)
                                    hundleItem(index)}}> ... <span className="txt-read">Read More</span></button>
                             )}
                            </td>
                            <td className="price"><span className="onMobile">Price: </span>{`${item.price} $`}</td>
                            <td className="icons">
